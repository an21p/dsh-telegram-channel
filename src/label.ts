...
export function displayLabel(parts: AgentLabelParts): string {
  ...
  // Only disambiguate with the workspace when there is no real title; otherwise
  // the workspace is redundant noise in the label.
  if (!parts.title && parts.workspace) bits.push(parts.workspace)
  if (bits.length === 0) bits.push(`session ${parts.index + 1}`)
  // idTail may already carry a leading ellipsis — don't double it.
  bits.push(parts.idTail.startsWith('…') ? parts.idTail : `…${parts.idTail}`)
