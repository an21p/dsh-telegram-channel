...
export class TelegramBridge {
    ...
    async onSessionEvent(session, event) {
        ...
        if (event.type === 'tool/result') {
            ...
            await this.forwardImages(targets, data?.message?.content, toolName ? `Screenshot from ${toolName}` : undefined);
            return;
        }
     ...
     * cannot see.
     */
    async readImageBytes(ref) {
        const id = String(ref.attachmentId ?? '');
        if (!/^[a-f0-9]{64}$/i.test(id))
            return undefined;
        try {
            const svc = this.ctx.attachments;
            const viaService = svc?.imageHostPath?.(ref);
            if (viaService)
                return await readFile(viaService);
        }
        catch (err) {
     ...
     * exact bytes.
     */
    async forwardImages(targets, content, caption) {
        ...
        for (const ref of fresh) {
            if (typeof ref.bytes === 'number' && ref.bytes > MAX_OUTBOUND_IMAGE_BYTES) {
                for (const b of targets) {
                    await this.enqueueNotice(b.chatId, `Image too large to forward (${Math.round(ref.bytes / 1024 / 1024)} MB).`);
                }
                continue;
            }
            let bytes;
